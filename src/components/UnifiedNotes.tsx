...
interface UnifiedNotesProps {
  notes?: UnifiedNotesData;                // ✅ optional 로 변경
  onUpdate?: (notes: UnifiedNotesData) => void; // ✅ optional 로 변경
}
...
export function UnifiedNotes({ notes, onUpdate }: UnifiedNotesProps) {
  ...
  // 2) 외부 props를 섞어서 safe 값 만들기
  const initialSafeNotes: UnifiedNotesData = {
    ...defaultNotes,
    ...(notes || {}),
  };

  // 3) 로컬 state로 통합 관리
  const [localNotes, setLocalNotes] = useState<UnifiedNotesData>(initialSafeNotes);
  const [activeTab, setActiveTab] = useState<TabType>("quick");

  // 4) 외부에서 notes가 바뀌면 localNotes도 동기화
  useEffect(() => {
    if (!notes) return;
    setLocalNotes((prev) => ({
      ...prev,
  ...
  // ========== 5C 파싱 ==========
  const parseFiveC = (quickNotes: string): FiveCData => {
    try {
      const parsed = JSON.parse(quickNotes || "{}");
      return {
        category: parsed.category || "",
        context: parsed.context || "",
        correctness: parsed.correctness || "",
  ...
  const [fiveCData, setFiveCData] = useState<FiveCData>(
    parseFiveC(initialSafeNotes.quickNotes)
  );

  useEffect(() => {
    // localNotes.quickNotes가 외부에서 바뀐 경우 5C도 다시 파싱
    setFiveCData(parseFiveC(localNotes.quickNotes));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [localNotes.quickNotes]);

  // ========= 공통 업데이트 헬퍼 =========
  const pushUpdate = (next: UnifiedNotesData) => {
    setLocalNotes(next);
    onUpdate?.(next); // 부모가 핸들러를 줬을 때만 호출
  };
      ...
      <div className="flex-1 overflow-auto p-6">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <label className="block">
                <span className="text-slate-700">1. 범주 (Category)</span>
                <span className="block text-slate-500 text-xs mt-0.5">
                  이 논문은 어떤 유형인가? 측정 논문인가? 기존 시스템에 대한
                  분석인가? 연구 프로토타입에 대한 설명인가?
                </span>
              </label>
              <textarea
                value={fiveCData.category}
            ...
            <div className="space-y-2">
              <label className="block">
                <span className="text-slate-700">2. 맥락 (Context)</span>
                <span className="block text-slate-500 text-xs mt-0.5">
                  어떤 다른 논문들과 관련이 있는가? 문제를 분석하기 위해 어떤
                  이론적 기반이 사용되었는가?
                </span>
              </label>
              <textarea
            ...
            <div className="space-y-2">
              ...
              <textarea
                value={fiveCData.correctness}
                onChange={(e) =>
                  handleFiveCChange("correctness", e.target.value)
                }
                placeholder="논문의 가정과 논리를 평가하세요..."
                className="w-full h-20 p-3 rounded-lg border border-indigo-200 focus:outline-none focus:ring-2 focus:ring-indigo-400 bg-indigo-50/20 resize-none text-sm"
              />
            ...
            <div className="space-y-2">
              ...
              <textarea
                value={fiveCData.contributions}
                onChange={(e) =>
                  handleFiveCChange("contributions", e.target.value)
                }
                placeholder="논문의 핵심 기여를 정리하세요..."
                className="w-full h-20 p-3 rounded-lg border border-indigo-200 focus:outline-none focus:ring-2 focus:ring-indigo-400 bg-indigo-50/20 resize-none text-sm"
              />
        ...
        {activeTab === "detailed" && (
          <div className="space-y-3">
            ...
            <textarea
              value={localNotes.detailedNotes}
              onChange={(e) => handleDetailedNotesChange(e.target.value)}
              placeholder="• 논문의 핵심 기여&#10;• 제안하는 방법론&#10;• 실험 설계 및 결과&#10;• 나라면 어떻게 구현할지&#10;• 차별점과 혁신 포인트"
              className="w-full h-96 p-4 rounded-lg border border-purple-200 focus:outline-none focus:ring-2 focus:ring-purple-400 bg-purple-50/30 resize-none"
            />
