...
//   return (
//     <div className="w-screen h-screen bg-gradient-to-br from-indigo-50 via-white to-purple-50 flex items-center justify-center p-4">      <div className="w-full max-w-md">
//         {/* Logo & Title */}
...
export function Login({ onLogin }: LoginProps) {
  ...
  const GOOGLE_CLIENT_ID = import.meta.env.VITE_GOOGLE_CLIENT_ID;
  const KAKAO_CLIENT_ID = import.meta.env.VITE_KAKAO_CLIENT_ID;
  const FE_CALLBACK_URL = import.meta.env.VITE_FE_CALLBACK_URL;
  ...
  const handleGoogleLogin = () => {
    ...
    window.location.href = url; // 구글 로그인 창으로 이동
  };

  const handleKakaoLogin = () => {
    const url = buildKakaoLoginUrl();
    window.location.href = url; // 카카오 로그인 창으로 이동
  };
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 border border-slate-200">
          ...
          <div className="space-y-3">
            ...
            >
              <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none">
                <path d="M12 3C6.477..." fill="#3C1E1E" />
              </svg>
              <span className="text-slate-900">카카오로 계속하기</span>
            </button>
